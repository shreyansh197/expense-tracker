...
export function ChartIllustration({ size = 140 }: { size?: number }) {
  return (
    ...
    >
      ...
      {/* Grid lines */}
      {[55, 70, 85, 100].map((y) => (
        <line key={y} x1="30" y1={y} x2="112" y2={y} stroke="var(--border)" strokeWidth="0.5" opacity="0.4" strokeDasharray="3 3" />
      ))}

      {/* Baseline */}
      <line x1="30" y1="105" x2="112" y2="105" stroke="var(--border)" strokeWidth="1" opacity="0.5" />
      <line x1="30" y1="40" x2="30" y2="105" stroke="var(--border)" strokeWidth="1" opacity="0.5" />
      ...
      <g style={{ animation: 'float-gentle 4s ease-in-out 0.5s infinite' }}>
        <rect x="88" y="34" width="28" height="16" rx="8" fill="var(--success-soft)" stroke="var(--accent)" strokeWidth="1" opacity="0.4" />
        <text x="102" y="45" textAnchor="middle" fontSize="8" fontWeight="bold" fill="var(--accent)" opacity="0.6">+12%</text>
      </g>

      {/* Sparkle */}
      <path d="M24 60L26 56L28 60L26 64Z" fill="var(--accent)" opacity="0.2" />
    </svg>
  );
}
