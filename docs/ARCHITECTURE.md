...
1. **Deterministic conflict resolution for money fields.** Extend the sync engine to per-field last-writer-wins with a user prompt on `amount`/`expectedAmount` collisions. Track resolution in the audit log. Ties into `useSyncConflictToast`. _(Also called out in the PRD.)_
