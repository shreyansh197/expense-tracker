...
- **Exponential backoff + jitter drain (T-2.2.2):** [src/lib/syncEngine.ts](../src/lib/syncEngine.ts) now filters `pushMutations` by `nextRetryAt`, promotes failed batches to `computeBackoffDelay(attempts)` (capped at 30 s + jitter), and schedules a targeted retry via `_scheduleRetryDrain` so recovery does not wait for the slow-poll tick. Existing `online` / `visibilitychange` handlers benefit automatically.
- **Idempotency migration & replay body (T-2.2.3):** New `prisma/migrations/014_mutation_idempotency.sql` adds `entity_id` to `processed_idempotency_keys`, formalises the named unique index, and re-enables RLS. `src/app/api/sync/commit/route.ts` stores the produced entity id on first apply and returns it on replay so two commits with the same key produce byte-for-byte identical `{ status: "applied", id }` bodies. Corresponding `ProcessedIdempotencyKey` model added to [prisma/schema.prisma](../prisma/schema.prisma); `ensureSyncColumns` mirrors the migration for hot deploys.
- **Dead-letter surface (T-2.2.4):** After `MAX_MUTATION_ATTEMPTS` (8) failures a mutation surfaces via new `getDeadLetterMutations`, `retryDeadLetter`, `discardDeadLetter`, `restoreDiscardedMutation`, and the `onDeadLetterChange` observable. `SyncDiagnosticsCard` renders a per-item panel with Retry / Discard actions, an Undo toast for discards, and audit-logged user actions via new `/api/audit/sync-dead-letter` route. Audit action union extended with `sync.dead_letter_retry` / `sync.dead_letter_discard`.
- **Reliability & idempotency tests (T-2.2.5):** New `src/__tests__/syncEngine.reliability.test.ts` (persistence across simulated tab close, failing-push backoff, workspace isolation, dead-letter lifecycle) and `src/__tests__/syncCommit.idempotency.test.ts` (replay body invariant, mixed-batch dedup). Both suites are green and form the M2 exit gate for the mutation-queue contract.
...
- **ConflictReviewSheet (T-2.3.2):** New [src/components/sync/ConflictReviewSheet.tsx](../src/components/sync/ConflictReviewSheet.tsx) subscribes to the money-conflict registry and opens a focus-trapped, reduced-motion [BottomSheet](../src/components/ui/BottomSheet.tsx) whenever a money conflict is pending. It shows both contested values side by side ("This device" vs "Other device", formatted via `useCurrency`) and resolves via deterministic Keep-mine / Keep-theirs actions (`resolveMoneyConflict`). Money is scalar, so no automatic-merge action is offered. Mounted globally in [src/app/providers.tsx](../src/app/providers.tsx). Design-tokens only, 48 px touch targets, keyboard operable.
- **Money-conflict audit trail (T-2.3.3):** `resolveMoneyConflict` tags its corrective upsert with `conflict: { field, choice }` (added to `IDBMutation` in [src/lib/db.ts](../src/lib/db.ts) and to every mutation schema in [src/lib/validators.ts](../src/lib/validators.ts)); `src/app/api/sync/commit/route.ts` emits a `conflict.resolve.money` [audit_logs](../src/lib/server/audit.ts) row capturing entity type, id, and chosen side only — never a monetary value (R-8). Best-effort (never fails an applied mutation); skipped on idempotent replays.
- **Money branded type + decimal-safe helpers (T-2.3.4):** New [src/lib/money.ts](../src/lib/money.ts) — `type Money = number & { __brand: "minor-units" }` with `toMinor`, `fromMinor`, `addMoney`, `subMoney`, `mulMoney`, `sumMoney`, `formatMoney`. All arithmetic runs in integer minor units, eliminating binary-float drift (`addMoney(0.1, 0.2) === 0.3`). Non-finite inputs throw. Closes TD-9.
- **Money-path migration (T-2.3.5):** All 108 raw money-arithmetic sites across 40 files (calculations, exchangeRates, filters, analytics/business/dashboard/expenses components, hooks, pages) migrated to the `money.ts` helpers. A colour-blend factor misnamed `amount` in [AccentColorPicker](../src/components/settings/AccentColorPicker.tsx) was renamed to `factor` (not money). Removes silent float rounding across analytics (R-5).
- **Money-arithmetic lint guard (T-2.3.6):** A `no-restricted-syntax` rule in [eslint.config.mjs](../eslint.config.mjs) (selectors shared via `eslint.money-rule.cjs`) forbids `+`/`-`/`*` and their compound forms on `amount`/`expectedAmount`/`receivedAmount` identifiers outside `src/lib/money.ts`; `error`-level so CI blocks regressions. Codifies TD-9.
- **Conflict + money test suites (T-2.3.7):** New `money.helpers.test.ts` (full-branch helper coverage), `syncEngine.conflict.test.ts` (two-client edit → pending conflict → deterministic resolution + audit signal), `conflictReviewSheet.contract.test.ts` (visual invariants), `moneyLintRule.test.ts` (guard fires on synthetic violations); `calculations.test.ts` extended with a Money-precision block.
